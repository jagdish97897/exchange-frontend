...
const UpdateUserProfile = ({ route }) => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch(`http://192.168.1.4:8000/api/v1/users/${phoneNumber}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.1.4:8000/api/v1/users/updateuser/${phoneNumber}`,
        {
          method: 'PUT',
          body: formData,
        }
...
//           console.log('condition formadata',formData);
//         }
  
//         const response = await fetch(`http://192.168.1.4:8000/api/v1/users/updateuser/${phoneNumber}`, {
//           method: 'PUT',
//           body: formData,
//         });
//         const result = await response.json();
