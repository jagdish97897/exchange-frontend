...
const CheckoutButton = ({ route }) => {
  ...
  const checkoutHandler = async () => {
    try {
      ...
      const { data: { order } } = await axios.post("http://localhost:8000/api/v1/visitor/placeOrder", { amount: formData.amount });
      ...
      const options = {
        ...
        order_id: order.id,
        callback_url: "http://localhost:8000/api/v1/visitor/verifyPayment",
        prefill: {
          name: formData.name,
