...
const CheckoutButton = () => {
  ...
  const checkoutHandler = async () => {
    try {
      ...
      // Create order
      const { data: { order } } = await axios.post("http://192.168.1.13:8000/api/v1/visitor/placeOrder", { amount: formData.amount });
      ...
      const options = {
        ...
        order_id: order.id,
        callback_url: "http://192.168.1.13:8000/api/v1/visitor/verifyPayment",
        prefill: {
          name: formData.name,
