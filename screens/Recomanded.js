...
const Recomanded = ({ route }) => {
    ...
    useEffect(() => {
        const fetchVehicleDetails = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/api/v1/visitorbikedetails/visitorbikedetails/${vehicleno}`);
                ...
                if (response.status === 200) {
                    ...
                    const kitsResponse = await axios.get('http://localhost:8000/api/kits/get');
    ...
    const addKit = async (kitId, kitName) => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:8000/api/cart/item', payload);
