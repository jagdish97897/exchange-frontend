...
const Recomanded = ({ route }) => {
    ...
    useEffect(() => {
        const fetchVehicleDetails = async () => {
            try {
                const response = await axios.get(`http://192.168.1.5:8005/api/v1/visitorbikedetails/visitorbikedetails/${vehicleno}`);
                ...
                if (response.status === 200) {
                    ...
                    const kitsResponse = await axios.get('http://192.168.1.5:8005/api/kits/get');
    ...
    const addKit = async (kitId, kitName) => {
        ...
        try {
            ...
            const response = await axios.post('http://192.168.1.5:8005/api/cart/item', payload);
