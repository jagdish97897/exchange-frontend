...
const UpdateVehicleScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const fetchVehicleDetails = async () => {
      try {
        const response = await fetch(`http://localhost:8000/api/vehicles/${vehicleNumber}`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/vehicles/update/${vehicleNumber}`, {
        method: "PUT",
        headers: {
