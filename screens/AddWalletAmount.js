...
// const AddWalletAmount = () => {
//   const route = useRoute();
//   const { userId } = route.params; // Ensure userId is passed correctly via navigation
//   const [amount, setAmount] = useState("");
...
//     try {
//       // Fetch Razorpay key
//       const { data: keyData } = await axios.get("http://192.168.1.9:8000/api/getkey");
//       const key = keyData?.key;
//       if (!key) {
//         throw new Error("Failed to fetch Razorpay key.");
//       }
...
//       // Create Razorpay order
//       const response = await axios.post("http://192.168.1.9:8000/api/wallet/checkout", { amount });
//       const order = response.data.order;
...
//           try {
//             const verificationResponse = await axios.post(
//               "http://192.168.1.9:8000/api/wallet/paymentVerification",
//               verificationData
//             );
