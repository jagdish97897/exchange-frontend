...
const TripScreen = ({ route }) => {
  ...
  const renderTripCard = (item) => {
    ...
    if (trip.status === 'inProgress' && trip.biddingStatus === 'accepted' && trip.grAccepted) {
    ...
    if (hasPaid80 && hasPaid10) {
      buttonLabel = 'View Details';
      shouldShowPayNow = false;
    }
  
    if (hasPaid80 && trip.billAccepted && hasPaid10) {
      paymentPercentage = 10;
      buttonLabel = 'Pay 10% Now';
      shouldShowPayNow = true;
    }
    // Ensure that after the last transaction, the button always shows "View Details"
if (hasPaid80 && hasPaid10 && trip.billAccepted) {
  buttonLabel = 'View Details';
  shouldShowPayNow = false;
}
